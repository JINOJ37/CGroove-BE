...
public class ClubController {
    ...
    public ResponseEntity<ApiResponse<ClubResponse>> createClub(
            ...
            @RequestPart(value = "clubImage", required = false) MultipartFile clubImage
    ) {
        ClubResponse clubResponse = clubService.createClub(userDetail.getUserId(), request, clubImage);
    ...
    public ResponseEntity<ApiResponse<ClubResponse>> updateClub(
            @AuthenticationPrincipal UserDetail userDetail,
            @PathVariable Long clubId,
            @RequestPart("request") @Valid ClubUpdateRequest request,
            @RequestPart(value = "clubImage", required = false) MultipartFile clubImage
    ) {
        ClubResponse clubResponse = clubService.updateClub(userDetail.getUserId(), clubId, request, clubImage);
